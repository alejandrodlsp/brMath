...
namespace BreathEngine
{
	...
	public:
		...
		/* Public methods */
		inline BR_FLOAT* GetElementsPointer() { return this->mElements; }
		...
		static Mat4 Rotate(BR_FLOAT angle, Vector3 rotation)
		{
			Mat4 newMatrix = Mat4();
			newMatrix = newMatrix.RotationX(angle * rotation.x);
			newMatrix = newMatrix.RotationY(angle * rotation.y);
			newMatrix = newMatrix.RotationZ(angle * rotation.z);
			return newMatrix;
		}
